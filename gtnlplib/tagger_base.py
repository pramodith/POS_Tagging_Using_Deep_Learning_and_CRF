...
def apply_model(model,outfilename,word_to_ix,char_to_ix, all_tags=None,trainfile=TRAIN_FILE,testfile=DEV_FILE):
    """
    ...
    with open(outfilename,'w') as outfile:
        for words,_ in preproc.conll_seq_generator(testfile):
            seq_words = bilstm.prepare_sequence(words, word_to_ix,char_to_ix)
            pred_tags = model.predict(seq_words)
            for i,tag in enumerate(pred_tags):
                outfile.write(tag+'\n')
            outfile.write('\n')

def eval_model(model,outfilename, word_to_ix,char_to_ix ,all_tags=None,trainfile=TRAIN_FILE,testfile=DEV_FILE):
    ...
    confusion_matrix -- dict of occurences of (true_label, pred_label)
    """
    apply_model(model,outfilename,word_to_ix, char_to_ix,all_tags,trainfile,testfile)
    return scorer.get_confusion(testfile,outfilename) #run the scorer on the prediction file
