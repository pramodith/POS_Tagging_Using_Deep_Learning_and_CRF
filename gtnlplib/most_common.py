...
def get_tag_word_counts(trainfile):
    ...
    for (words,tags) in gen:
        for word,tag in zip(words,tags):
            all_counters[tag].update({word:1})
    return all_counters
...
def get_most_common_word_weights(trainfile):
    ...
    all_counts=get_tag_word_counts(trainfile)
    all_tags=list(all_counts.keys())
    tag_word={}
    for tag in all_counts:
        for word in all_counts[tag]:
            if dup_cnt[word]<all_counts[tag][word]:
                dup_cnt[word]=all_counts[tag][word]
                tag_word[word]=tag
     #       sum.update({tag:all_counts[tag][word]})
    #weights=defaultdict(lambda :{sum.most_common(1)[0]:1},weights)
    for word in tag_word:
        weights[(tag_word[word],word)]=1
        for tag in all_tags:
            if tag!=tag_word[word]:
                weights[(tag,word)]=0
    return weights
